...
bootstrap
```{r}
bootstrap_time <- tibble(experiment = rep(1:10000, each =100),
                        ind = sample(1:100, size = 100*1000, replace = TRUE),
                                     ystar = q3_company[ind])
bias <- bootstrap_time %>% 
  group_by(experiment) %>% 
  summarise(delta = mean(y) - mean(ystar))
mean(y) + quantile(bias$delta, c(0.05, 0,95))
summarise(delta = median_function(df_median) - median_function(survfit(Surv(timestar, churnstar) ~ experiment)))
