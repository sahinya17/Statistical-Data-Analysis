...
```{r}
folds <- vfold_cv(A2, v = 20, strata = region)
folds
...
```{r}
nm_rec <- recipe(logr ~ logu5 + region + t,
              data = A2_training) %>% 
  step_bs(logu5, deg_free = tune()) %>% 
  step_dummy(region) %>% 
  step_interact(~starts_with("logu5"):starts_with("region"))
```
