...
```{r echo = TRUE, message = FALSE, warning = FALSE, variable-choice, fig.width=20, fig.height=15}
...
lm_pred %>% 
  metrics(truth = transformed_nmr,
          estimate = pred) %>% 
  kbl(booktabs = T,
      caption = "Mean absolute error, Root mean Square Error, R-squared value for the test set data") %>%
...
coverage <- function(data, formula, n, n_sim,ci){
  spec_lasso_tune <- linear_reg(penalty = tune(), mixture = 1) %>% 
  set_engine("glmnet")
  
  lambda <- grid_regular(penalty(range = c(-10,10)),
                         levels = 10)
...
```{r echo = TRUE, message = FALSE, warning = FALSE}
lasso(data, formula, n, n_sim) %>% tidy() %>% 
  kbl(booktabs = T,
      caption = "80% confidence interval for Lasso") %>%
  kable_styling(position = "center")

coverage(data, formula, n, n_sim, ci) %>% 
  kbl(booktabs = T,
      caption = "Coverage check for each interval") %>%
  kable_styling(position = "center")
```

The modified residual bootstrap does not achieve the nominal coverage.  
