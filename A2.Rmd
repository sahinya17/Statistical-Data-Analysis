...
top3 <- nmr_data %>% filter(region %in% c("Central Europe, Eastern Europe, Central Asia", "High income", "Southeast Asia, East Asia and Oceania")) %>% 
...
A2_PI <- as_tibble(predict(fit, nmr_test, interval = "prediction")) %>% 
  bind_cols(lm_pred)
