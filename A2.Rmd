...
```{r}
fit_non_linear <- lm(transformed_nmr ~ bs(u5mr_log, df= 8) + u5mr_log*region + u5mr_log*time, data = nmr_train)
```
...
The first model is a linear model while the second is a non-linear model with degree of freedom of 8. 

The non-linear model is more appropraite model of the data
